...
//    for(int x=0; x<=prevsum+1; x++){
    for(int x=0; x<=min(prevsum+1, maxanalysis[y]); x++){
      if(tbl[y-1][x] > 0)
          //check all blocks
...
//            if( (y==colors.size()-1) && (cntminus==0) ){
//              cout << "x:" << x << ", ij:" << ij << ", cminus:" << cminus << endl;
//            }
//            cout << "y:" << y << ", x:" << x << ", ij:" << ij << ", cminus:" << cminus << ", cntminus:" << cntminus << endl;
//            cout << "y:" << y << ", x:" << x << ", ij:" << ij << ", cminus:" << cminus << ", cntminus:" << cntminus << endl;
            cout << y << ", " << x << ", " << ij << ", " << cminus << ", " << cntminus << endl;
          }
      }
    }
...
//            //all "-" count
////          for(int cminus = 0; cminus <= min(ij, x); cminus++){
//          int cminus = x + colors[y] - ij;  //only for 0 element
//          if( cminus <= min(ij, x) )
//          {
//            int cplus = ij - cminus;
//            int oplus = prevsum + 1 - x;
